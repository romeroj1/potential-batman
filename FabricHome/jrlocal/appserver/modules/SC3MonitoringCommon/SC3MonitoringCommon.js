var SC3Monitoring = {
		...
		*/
		...
		if (search) {
			if (token == "search") {
				...
				console.log("WARNING: SC3Monitoring was asked to find value for reserved token that is unreserved, empty string will be sub'ed for token:" + token);
				return "";
			}
		}
		console.log("WARNING: SC3Monitoring was asked to find value for reserved token with no search in context, it will be replaced with empty string, token:" + token);
		return "";
	},
		...
		*/
		...
		if (appVars) {
			for (var ii=0; ii<replacementTokens.length; ii++) {
				var token = replacementTokens[ii];
				if ($.inArray(token,this.reservedTokens) > -1) {
					var replacer = new RegExp("\\$" + this.escapeStringForRegExp(token) + "\\$");
					var value = this.getReservedTokenValue(token, context);
	...
	storeVariable: function(displayField, valueField, value, context) {
		...
		*/
		...
		if (valueField) {
			var name = displayField + "." + valueField;
		}
		else {
			var name = displayField;
		}
		//Construct RegExp
	...
	*/
	jobRegistry: {}, 
	registerJobProgress: function(key, progress) {
		/** 
	...
	getJobProgress: function(key) {
		...
		*/
		if (this.jobRegistry.hasOwnProperty(key)) {
			...
			console.log("[SC3Monitoring GLOBAL] ERROR: progress was requested on a key that is not registered with SC3Monitoring, " + key);
			return 0.0;
		}
	},
	registerJobDone: function(key, url) {
		/** 
	...
	lookupSavedSearch: function(name, nameSpace, context) {
		...
		var targetArtifact = $.parseJSON($.ajax({
			url : ('/lists/entities/admin/savedsearch/'+name+'?output_mode=json&namespace='+nameSpace),
			dataType : "json",
			async : false,
		...
		if ( ajaxResponse == "success" && targetArtifact.length > 0 ) {
			if (targetArtifact[0].hasOwnProperty('search')) {
				...
				searchString = SC3Monitoring.replaceVariables(searchString, context);
			} else {
	...
	parseResults: function (results) {
		...
		if (parseInt(splunkVersion.charAt(0)) >= 5) {
			if ( results.length === 0 ) {
	...
	/**
	*hello I'm splunk applications here with SC3Monitoring!
	*are you tired of your fucking numbers having too many decimal places?
	*wish you didn't have to write custom code to round things for display purposes?
	*YOU NEED SC3Monitoring!
	*now with SC3Monitoring.roundNumber you can get a number back rounded to any number of decimal places you want!
	*but wait I always want it to two places...
	*LOOK NO FURTHER, SC3Monitoring.roundNumber will default to 2 places if you don't tell it how to round just type:
	*roundedNumber = SC3Monitoring.roundNumber(num); 
	...
	*/
		...
		*/
		...
		for (var ii=0; ii<mList.length; ii++) {
			...
			if (this.startswith(tmpMod.moduleType,"Splunk.Module.")) {
				var tmpType = tmpMod.moduleType.split(".")[2];
			}
			else {
				var tmpType = tmpMod.moduleType;
			}
			if (tmpType == type) {
				matches.push(tmpMod);
			}
	...
	keyListener: function(event) {
		...
		{
			if (SC3Monitoring.anubisWindow === "" || SC3Monitoring.anubisWindow.closed || SC3Monitoring.anubisWindow.name == undefined) {
				SC3Monitoring.anubisWindow = window.open("/custom/SA-Utils/anubis_service/" + Splunk.util.getCurrentApp() + "/show?view=" + Splunk.util.getCurrentView(),"subWindow","HEIGHT=775,WIDTH=1400");
			}
			else {
				SC3Monitoring.anubisWindow.focus();
			}
		}
	}
};
//Event Bindings for SC3Monitoring Utilities
//Enable hotkeys
$(document).keydown( SC3Monitoring.keyListener );

Splunk.Module.SC3MonitoringCommon = $.klass(Splunk.Module, {
	initialize: function($super, container) {
		$super(container);
		console.log("loaded common tools into var SC3Monitoring");
	...
	getModifiedContext: function (context) {
		...
		for (var name in this.qsVars) {
			SC3Monitoring.storeVariable(name, null, this.qsVars[name], context);
		}
		return context;
	}
});
